...
export const getUser = async (authId: string): Promise<User | null> => {
  try {
    ...
    const response = await fetch(`${auth0BaseUrl}api/v2/users/${authId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to get user: ${errorBody.message}`);
    }
